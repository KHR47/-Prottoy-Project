...
import { Activity, Search, MapPin, History, PencilLine, CheckCircle, X, ShieldAlert } from "lucide-react";
...
type Meter = {
  id: string; // Temporarily empty or generated ID for pending
  citizenName: string;
  zone: string;
...
export default function MeterManagementPage() {
  ...
  const handleApproveSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.patch(`/water/meters/${approveMeterReq.id}/approve`, {
      ...
      setHardwareId("");
      setMaxLimit(1000);
      toast.success("Meter request approved successfully.");
    } catch (err) {
  ...
  const handleReadingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/water/meters/${readingMeterId}/readings`, {
        value: newReading,
        recordedBy: user?.name || "Officer"
      });
      
      let msg = `Reading of ${newReading}L logged for ${readingMeterId}`;
      if (meter.maxLimit && newReading > meter.maxLimit) {
         msg += `. WARNING: Exceeds max limit of ${meter.maxLimit}L!`;
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-slate-200 pb-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-bold uppercase text-blue-700">Water Infrastructure</p>
              <h1 className="text-3xl font-black text-slate-950">Meter Management</h1>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-col sm:flex-row justify-between gap-4">
          ...
          <div className="relative w-full sm:max-w-xs">
            ...
            <input
              ...
              className="block w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-3 text-sm placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 shadow-sm"
            />
          </div>
        </div>
            ...
            <table className="w-full min-w-[1000px] text-left text-sm">
              <thead className="bg-slate-100 text-xs uppercase font-semibold text-slate-600 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-4">Limit / Usage</th>
                    ...
                    <tr key={meter.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {meter.status === "pending" ? (
                          ...
                          <div>
                            <p className="font-mono text-slate-900 font-bold">{meter.lastReading} L</p>
                            <p className="text-xs text-slate-500">Max: {meter.maxLimit} L</p>
                        ...
                        {meter.status === "pending" ? (
                          <button 
                            onClick={() => {
                              ...
                              setHardwareId(`WM-${zoneAbbr}-${rand}`);
                            }}
                            className="flex items-center justify-center gap-1 rounded-lg bg-blue-600 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-blue-700 transition ml-auto"
                          >
                          ...
                          <div className="flex justify-end gap-2">
                            <button 
                              ...
                              className="flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 shadow-sm hover:bg-slate-50 hover:text-blue-600 transition"
                            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 bg-slate-50">
              <h2 className="text-xl font-black text-slate-900">Approve Meter Request</h2>
            ...
            <form onSubmit={handleApproveSubmit} className="p-6">
              ...
              <div className="mb-6 bg-blue-50 border border-blue-100 rounded-xl p-4">
                <p className="text-xs font-bold text-blue-800 uppercase mb-1">Citizen Details</p>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input required placeholder="e.g. WM-XYZ-123" value={hardwareId} onChange={e => setHardwareId(e.target.value)} className="w-full border border-slate-300 rounded-lg p-3 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 font-mono" />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-bold text-slate-700">Set Monthly Limit (Liters)</label>
                  <div className="relative">
                    <input type="number" required min={100} value={maxLimit} onChange={e => setMaxLimit(parseInt(e.target.value) || 0)} className="w-full border border-slate-300 rounded-lg p-3 text-sm outline-none focus:border-blue-500 font-mono pr-12" />
              ...
              <div className="mt-8 flex gap-3">
                ...
                <button type="submit" className="w-2/3 rounded-xl bg-blue-600 py-3 text-sm font-bold text-white transition hover:bg-blue-700 flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleReadingSubmit} className="p-6">
              ...
              <div>
                <label className="mb-1 block text-sm font-bold text-slate-700">New Reading (Liters)</label>
                <input 
                  type="number" 
                  required 
                  min={0}
                  ...
                  className="w-full border border-slate-300 rounded-lg p-3 text-lg font-mono outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" 
                />
              </div>
              <div className="mt-6">
                  ...
                  {activeHistoryMeter.readings.map((reading, i) => (
                    <div key={reading.id} className="relative pl-6">
                      ...
                      <div className="rounded-lg border border-slate-200 p-4 shadow-sm">
                        ...
                        <p className="text-lg font-mono font-black text-slate-700">{reading.value} L</p>
                      </div>
                    </div>
                  ))}
                </div>
