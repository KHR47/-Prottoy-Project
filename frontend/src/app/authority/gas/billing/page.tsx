...
export default function GasBillingSystemPage() {
  const { isReady } = useRequireRole(["authority", "admin"]);
  const [meters, setMeters] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  // Dynamic configuration state for Gas
  const [config, setConfig] = useState({
    tier1Limit: 30, // m3
    tier1Rate: 15.0, // Taka per m3
    tier2Limit: 60,
    tier2Rate: 20.0,
    tier3Rate: 30.0,
    penaltyFee: 1000
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      "Meter ID", "Citizen Name", "Zone",
      "Usage (m\u00b3)", "Max Limit (m\u00b3)",
      "Base Charge (BDT)", "Penalty (BDT)", "Total (BDT)", "Status"
    ];
    ...
    const rows = meters.map((m) => {
      ...
      return [
        ...
        bill.penalty,
        bill.total,
        m.pendingInvoice ? "Issued" : "Pending"
      ];
    });
    ...
    link.href = url;
    link.download = `gas-billing-${date}.csv`;
    link.click();
    URL.revokeObjectURL(url);
    toast.success(`Exported ${rows.length} records to gas-billing-${date}.csv`);
  };

  const handleIssueInvoice = async (meterId: string, amount: number, breakdown: any) => {
    try {
      await api.post(`/gas/meters/${meterId}/invoice`, { amount, breakdown });
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 border-b border-slate-200 pb-5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-sm font-bold uppercase text-emerald-600">Gas Finance Hub</p>
          </div>
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-3xl font-black text-slate-900">{totalVolume.toLocaleString()} m³</p>
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm flex items-center gap-4">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
          <div className="border-b border-slate-200 bg-slate-50 p-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">Tier 1: {config.tier1Rate}৳/m³ | Tier 2: {config.tier2Rate}৳/m³ | Penalty: {config.penaltyFee}৳</p>
            </div>
            <button
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50/50 text-slate-500">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold">Usage (m³)</th>
                  ...
                  billingData.map((data) => {
                    ...
                    return (
                      <tr key={data.id} className={`transition-colors ${isIssued ? 'bg-emerald-50/30' : 'hover:bg-slate-50'}`}>
                        ...
                        <td className="px-6 py-4">
                          <span className="font-bold text-slate-700">{data.usage} m³</span>
