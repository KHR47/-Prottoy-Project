...
export default function MyGasUsagePage() {
      ...
      <div className="min-h-screen bg-slate-50 pb-12">
        ...
        <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
            ...
            <h2 className="text-xl font-bold text-slate-900">No Gas Meter Found</h2>
            <p className="text-slate-500 max-w-md mx-auto mt-2 mb-6">We couldn't find an active line gas connection registered to your account.</p>
            <Link href="/gas/request-meter" className="inline-flex rounded-lg bg-orange-600 px-6 py-3 font-bold text-white transition hover:bg-orange-700">
              Request Connection
            </Link>
          </div>
        </main>
  ...
  // Build consumption delta data: each bar = how much was used between consecutive readings
    ...
    <div className="min-h-screen bg-slate-50 pb-12">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 border-b border-slate-200 pb-5">
          <p className="text-sm font-bold uppercase text-orange-700">Gas Services</p>
          <h1 className="text-3xl font-black text-slate-950">My Gas Usage</h1>
        ...
        {hasAnomaly && (
          <div className="mb-8 rounded-xl border border-rose-200 bg-rose-50 p-5 flex gap-4 shadow-sm items-start">
              ...
              <div>
                  <h3 className="font-bold text-rose-900">Safety Limit Exceeded</h3>
                  <p className="text-sm text-rose-700 mt-1">
                      Your usage has exceeded the maximum monthly threshold of <strong>{meter.maxLimit}m³</strong> set by the authority. You may face penalty rates on your next billing cycle, or your supply may be throttled. We recommend checking your appliances or reducing consumption.
                  </p>
              </div>
          </div>
        )}
        ...
        <section className="mb-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-xl font-black text-slate-950">Consumption History (m³)</h3>
              <p className="text-sm text-slate-500 font-mono mt-1">Meter ID: {meter.id} • Zone: {meter.zone}</p>
            </div>
            {usageData.length > 1 && (
              <div className="flex items-center gap-2 text-sm font-bold text-orange-600 bg-orange-50 px-3 py-1.5 rounded-full">
              ...
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                <BarChart data={usageData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: any) => [`${value} m³`, 'Consumed']}
                  ...
                  >
                    {usageData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.isAnomaly ? "#e11d48" : "#f97316"} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {usageData.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-slate-500">No readings logged yet.</td>
                  </tr>
                ) : (
                  [...usageData].reverse().map((record: any, index: number) => (
                    <tr key={record.id || index} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right font-mono text-slate-600">{record.rawValue} m³</td>
                      <td className={`px-6 py-4 text-right font-mono font-bold ${record.isAnomaly ? 'text-rose-600' : 'text-slate-800'}`}>
                        {record.usage} m³
                      </td>
