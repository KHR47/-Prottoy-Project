...
export default function MyBillsPage() {
  ...
  const [paymentTab, setPaymentTab] = useState("mobile"); // mobile, card, bank
    ...
    if (isReady) {
      ...
      const fetchMyMeter = async () => {
        try {
          ...
          // Find the meter belonging to this citizen
  ...
  const handlePay = async (invoiceId: string) => {
    ...
    setTimeout(async () => {
      try {
        await api.patch(`/water/meters/${meter.id}/pay`);
    ...
    return (
      <div className="min-h-screen bg-slate-50 pb-12">
        ...
        <main className="mx-auto max-w-3xl px-4 py-12 text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-900">No Meter Found</h2>
          <p className="mt-2 text-slate-500">You do not have an active water connection. Please request a meter first.</p>
        </main>
      </div>
    );
  }
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 border-b border-slate-200 pb-5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-sm font-bold uppercase text-blue-700">Digital Payments</p>
          </div>
          <h1 className="text-3xl font-black text-slate-950">My Water Bills</h1>
          <p className="mt-2 text-slate-600">
            View your current utility invoices, pay online securely, and review your payment history.
          </p>
        </div>
        ...
        <section className="mb-10">
          ...
          {!invoice ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center shadow-sm">
              ...
              <p className="text-slate-500 mt-1">There are no pending water bills for your account.</p>
            </div>
          ) : (
              ...
              <div className="p-6 sm:p-8 bg-slate-50 flex flex-col md:flex-row gap-6 justify-between items-center">
                ...
                <div className="w-full md:w-auto">
                  <button 
                    ...
                    className="w-full md:w-auto flex items-center justify-center gap-2 px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-md transition"
                  >
            ...
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
              ...
              <div className="w-full md:w-64 bg-slate-50 border-r border-slate-200 shrink-0">
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-6 py-4 text-left font-bold border-b border-slate-200 transition ${paymentTab === "mobile" ? "bg-white text-blue-600 border-l-4 border-l-blue-600" : "text-slate-600 hover:bg-slate-100"}`}
                >
                  <Smartphone className="h-5 w-5" /> Mobile Banking
                </button>
                <button 
                  onClick={() => setPaymentTab("card")}
                  className={`w-full flex items-center gap-3 px-6 py-4 text-left font-bold border-b border-slate-200 transition ${paymentTab === "card" ? "bg-white text-blue-600 border-l-4 border-l-blue-600" : "text-slate-600 hover:bg-slate-100"}`}
                >
                  <CreditCard className="h-5 w-5" /> Credit / Debit Card
                </button>
                <button 
                  onClick={() => setPaymentTab("bank")}
                  className={`w-full flex items-center gap-3 px-6 py-4 text-left font-bold border-b border-slate-200 transition ${paymentTab === "bank" ? "bg-white text-blue-600 border-l-4 border-l-blue-600" : "text-slate-600 hover:bg-slate-100"}`}
                >
              ...
              <div className="p-6 md:p-8 flex-1 overflow-y-auto">
                ...
                {paymentTab === "mobile" && (
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="e.g. 017XXXXXXXX" className="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                    </div>
                  </div>
                )}
                ...
                {paymentTab === "card" && (
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="XXXX XXXX XXXX XXXX" className="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Expiry Date</label>
                        <input type="text" placeholder="MM/YY" className="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">CVV</label>
                        <input type="text" placeholder="123" className="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                      </div>
                    </div>
                  </div>
                )}
                ...
                {paymentTab === "bank" && (
                  <div className="space-y-4">
                    ...
                    <select className="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-white">
